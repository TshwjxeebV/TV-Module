...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateNextButton() {
        if(currentGlobalSlide === 6 || currentGlobalSlide === 7 || currentGlobalSlide === 9) return; 
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowRight') {
            const controller = window.slideControllers[currentGlobalSlide];
            if (controller && controller.nextStep && controller.nextStep()) { uiSound.playStepForward(); return; }
            if (currentGlobalSlide < totalSlides - 1 && slideCompletion[currentGlobalSlide] && currentGlobalSlide !== 6 && currentGlobalSlide !== 7 && currentGlobalSlide !== 9) goToSlide(currentGlobalSlide + 1);
    ...
    document.getElementById('btn-restart').addEventListener('click', () => {
        ...
        slideCompletion = [true, false, false, false, false, false, true, false, true, true];
    ...
    const masterScenarioBank = [
        {
            ...
            inspire: "Exactly. When I'm in a dungeon, I want the shadows to be pitch black, not glowing grey.", solve: "That sounds like exactly what my rig needs to push those graphics.", aftermath: { "oled": "KEPT. Marcus is mind-blown by the absolute black levels and instant response time. Perfect recommendation.", "miniled": "RETURNED. Marcus complained the blooming around his crosshairs in the dark room drove him crazy.", "led": "RETURNED. The standard LED was a massive bottleneck for his expensive graphics card. He lost trust in your advice.", "qled": "RETURNED. The colors were great but the grey blacks ruined his immersion." }
        },
        {
            ...
            inspire: "Yes! The glare on Sunday afternoons is awful right now. We want to actually see the screen without fighting the sun.", solve: "That sounds perfect. I just need it to work and be bright enough for the kids.", aftermath: { "miniled": "KEPT. Elena loves that they can finally see the screen clearly on sunny afternoons without closing the blinds.", "oled": "RETURNED. The TV was too dim to fight the window glare, and she panicked about the cartoon logo causing burn-in.", "led": "KEPT. She didn't return it, but complains the picture looks washed out. You missed out on premium margin.", "qled": "KEPT. It handles the sunroom decently well, but she wishes it were just a little punchier during the day." }
        },
        {
            ...
            inspire: "Oh, that sounds lovely. My eyes aren't what they used to be, so clear and simple is best.", solve: "Perfect. I just want something reliable that I don't have to worry about maintaining or overpaying for.", aftermath: { "led": "KEPT. The TV runs reliably all day. Walter is thrilled with your honest, practical recommendation.", "miniled": "KEPT. He likes it, but the extra brightness actually hurts his eyes at night. An unnecessary upsell.", "oled": "RETURNED (ANGRY). The news channel ticker permanently burned into the screen after 3 weeks.", "qled": "KEPT. The colors pop nicely for his classic movies without breaking his budget." }
        },
        {
        ...
        },
        {
            ...
            inspire: "Absolutely, my fans need to see the score from the back of the room through all the glare.", solve: "I need durability and punch. If it cuts through the glare and lasts 14 hours a day, I'm sold.", aftermath: { "miniled": "KEPT. The fans can clearly see the games from the street, driving massive foot traffic into the bar.", "oled": "RETURNED (LAWSUIT). The static scoreboards caused severe burn-in in just two weeks.", "led": "RETURNED. The TV washed out completely under the bar's neon lights.", "qled": "KEPT. It handles the glare decently well and provides a massive, colorful image for the patrons." }
        }
    ];
    ...
    function handleChoice(id, opt) {
        ...
        if (opt.phase === "TRANSITION") {
            ...
            if (id === "miniled") { tvName = "Premium Mini-LED TV"; techReason = "its thousands of dimming zones will effortlessly cut through the glare while keeping the contrast sharp"; }
    ...
    // --- SLIDE 8: DYNAMIC DISPLAY COMPARISON CONTROLLER ---
    const displayProfiles = {
        oled: { className: 'oled-profile', nit: 'Peak: ~800 nits', contrast: 'Contrast: Infinite : 1', hasBloom: false },
        miniled: { className: 'miniled-profile', nit: 'Peak: ~2000+ nits', contrast: 'Contrast: High (Dimming Zones)', hasBloom: true },
        qled: { className: 'qled-profile', nit: 'Peak: ~1200 nits', contrast: 'Contrast: Moderate (Uniform Backlight)', hasBloom: true }
    };

    function bindComparisonPanel(selectId, screenId, bloomId, nitId, contrastId) {
        const selectEl = document.getElementById(selectId);
        const screenEl = document.getElementById(screenId);
        const bloomEl = document.getElementById(bloomId);
        const nitEl = document.getElementById(nitId);
        const contrastEl = document.getElementById(contrastId);

        if (!selectEl || !screenEl) return;

        selectEl.addEventListener('change', (e) => {
            const profile = displayProfiles[e.target.value];
            if (!profile) return;
            uiSound.playToggleState(true);

            screenEl.className = `screen-simulator ${profile.className}`;
            bloomEl.style.display = profile.hasBloom ? 'block' : 'none';
            nitEl.innerText = profile.nit;
            contrastEl.innerText = profile.contrast;
        });
    }

    bindComparisonPanel('select-display-a', 'screen-a', 'bloom-a', 'spec-nit-a', 'spec-contrast-a');
    bindComparisonPanel('select-display-b', 'screen-b', 'bloom-b', 'spec-nit-b', 'spec-contrast-b');
